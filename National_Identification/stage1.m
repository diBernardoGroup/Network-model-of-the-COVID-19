...
while t_curr<=t_end
    ...
    [pars,y,If]=id_and_sim(tab_data,t_init,Data_sep,initial_guess,N,total_active);
    ...
    [~,y2]=id_and_sim(tab_data,Data_sep,t_curr,initial_guess,N,total_active);
    ...
    [p3,y3]=id_and_sim(tab_data,t_init,t_curr,initial_guess,N,total_active);   
    
    
    %Find if there is a change and where it occurred
    [Change,Where]=Find_Change(tab_data(:,t_init:t_curr),y,y2,y3,N,@(t) interp1(1:t_end-t_init+1,total_active_data(t_init:t_end),t),p3);
    ...
    if(Change==1)
       ...
       [pars,~,If]=id_and_sim(tab_data,t_init,t_change(end),initial_guess,N,total_active);   
       It0=If;
...
if t_change(end)<t_end
    ...
    [pars,~,If]=id_and_sim(tab_data,t_change(end),t_end,initial_guess,N,total_active);   
    [v,tau,g,I0]=deal(pars{:});
